function [Surfl,Surfr] = Create_Lobar_ConvexHull(Image);
%
% Syntax :
%   [Surfl,Surfr] = Create_Lobar_ConvexHull(Image);
%
% This script creates hemispheric convex hull from freesurfer parcellation
% atlas. After, each convex hull is labelled according to freesurfer Lobar
...
IA = I*0;
T = I(:);
ind = ismember(T,[ 4 28 5 30 31 SubCL FroIds TempIds ParIds OccIds 1000  1002 1010 1023 1026 1035 ]');
...
IA(logical(It)) = 1;
%----------------------------------
ind = ismember(T,[43 44 60 62 63 SubCR FroIdsR TempIdsR ParIdsR OccIdsR 2000  2002 2010 2023 2026 2035 ]');
IA(ind) = 2; % Left Hemisphere==2 Gray Matter, Avoiding Freesurfer errors in white Matter Parcelation
% White Matter
ind = find(I == 5002);
It = IA*0;
It(ind) = 2;
...
Iseg = logical(I);
ind = ismember(T,[15 16 7 8 46 47]');
Iseg(ind) = 0; 
%% ===================================================================== %%
%% Mask Refilling 
for z = 1:size(Iseg,3)
...
[IA] = Atlas_Corr(Iseg,IA); % Filling CSF Parts
...
ind = find(IA == 1);
It(ind) = 1;
[Surf] = Surf_Extraction_custom(V,It);
Surfa(1) = Surf;
%% ===================================================================== %%
...
%% ================= Convex Hull Lobar Parcelation =======================%
% ---------------- Left Hemisphere ---------------------------------------%
It = IA*0;
T = I(:);
ind = ismember(T,[ 4 28 5 30 31 SubCL FroIds TempIds ParIds OccIds 1000  1002 1010 1023 1026 1035 250 251 252 253 254 255]');
It(ind) = I(ind);
%% Creating Left Lobes
% 
T = It(:);
Itaux = It*0;
ind = ismember(T,FroIds'); Itaux(ind) = 1; % Frontal
ind = ismember(T,TempIds'); Itaux(ind) = 2;  % Temporal
ind = ismember(T,ParIds'); Itaux(ind) = 3; % Parietal
ind = ismember(T,OccIds'); Itaux(ind) = 4; % Occipital
ind = ismember(T,[1000  1002 1010 1023 1026]'); Itaux(ind) = 5; % Cingulate
ind = ismember(T,1035); Itaux(ind) = 6; % Insula
ind = ismember(T,[250 251 252 253 254 255]'); Itaux(ind) = 7; % Corpus Callosum
ind = ismember(T,[4 28 5 30 31 SubCL ]'); Itaux(ind) = 8; % Subcortical Structures, Ventricles and CSF
%%

vert1 = Surfa(1).SurfData.vertices;
vert2 = Surfa(2).SurfData.vertices;

Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3)));Itaux = imdilate(Itaux,strel(ones(3,3,3)));
Is = spm_sample_vol(Itaux,double(vert1(:,1)),double(vert1(:,2)),double(vert1(:,3)),0);
Surfa(1).Is = Is;
% --------------- Right Hemisphere ---------------------------------------%
It = IA*0;
T = I(:);
ind = ismember(T,[43 44 60 62 63 SubCR FroIdsR TempIdsR ParIdsR OccIdsR 2000  2002 2010 2023 2026 2035 250 251 252 253 254 255]');
It(ind) = I(ind);

%% Creating Right Lobes
% 
T = It(:);
Itaux = It*0;
ind = ismember(T,FroIdsR'); Itaux(ind) = 1; % Frontal
ind = ismember(T,TempIdsR'); Itaux(ind) = 2;  % Temporal
ind = ismember(T,ParIdsR'); Itaux(ind) = 3; % Parietal
ind = ismember(T,OccIdsR'); Itaux(ind) = 4; % Occipital
ind = ismember(T,[2000  2002 2010 2023 2026]'); Itaux(ind) = 5; % Cingulate
ind = ismember(T,2035); Itaux(ind) = 6; % Insula
ind = ismember(T,[250 251 252 253 254 255]'); Itaux(ind) = 7; % Corpus Callosum
ind = ismember(T,[43 44 60 62 63 SubCR ]'); Itaux(ind) = 8; % Subcortical Structures, Ventricles and CSF
%%
%% Creating Right Lobes
Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3))); Itaux = imdilate(Itaux,strel(ones(3,3,3)));Itaux = imdilate(Itaux,strel(ones(3,3,3)));
Is = spm_sample_vol(Itaux,double(vert2(:,1)),double(vert2(:,2)),double(vert2(:,3)),0);
Surfa(2).Is = Is;
ind = find(Surfa(1).Is == 6);
Surfa(1).Is(ind) = 0;
[Surft] = Surf_Corr(Surfa(1));
% Surft = Surfa(1);
voxs = (V.mat*[Surft.SurfData.vertices(:,1) Surft.SurfData.vertices(:,2) Surft.SurfData.vertices(:,3) ones(size(Surft.SurfData.vertices(:,1),1),1)]')';
Surft.SurfData.vertices = voxs(:,1:3);
Surfa(1) = Surft;
[Surfl] = Struct_Area_Comp(Surfa(1));

ind = find(Surfa(2).Is == 6);
Surfa(2).Is(ind) = 0;
[Surft] = Surf_Corr(Surfa(2));
%Surft = Surfa(2);
voxs = (V.mat*[Surft.SurfData.vertices(:,1) Surft.SurfData.vertices(:,2) Surft.SurfData.vertices(:,3) ones(size(Surft.SurfData.vertices(:,1),1),1)]')';
Surft.SurfData.vertices = voxs(:,1:3);
Surfa(2) = Surft;
...
Itaux = imdilate(Itaux,strel(ones(3,3,3))); 
Itaux = imdilate(Itaux,strel(ones(3,3,3))); 
Itaux = imdilate(Itaux,strel(ones(3,3,3))); 
Itaux = imdilate(Itaux,strel(ones(3,3,3))); 
Itaux = imdilate(Itaux,strel(ones(3,3,3)));
...
%% Correcting Surfaces
ind = find(ismember(Surfa(1).Is(:,2), [250 251 252 253 254 255 4 28 5 30 31 SubCL ]'));
Surfa(1).Is(ind,2) = 0;
Surftemp.SurfData.vertices = Surfa(1).SurfData.vertices;
Surftemp.SurfData.faces = Surfa(1).SurfData.faces;
Surftemp.Is = Surfa(1).Is(:,2);
[Surft] = Surf_Corr(Surftemp);
voxs = (V.mat*[Surft.SurfData.vertices(:,1) Surft.SurfData.vertices(:,2) Surft.SurfData.vertices(:,3) ones(size(Surft.SurfData.vertices(:,1),1),1)]')';
Surft.SurfData.vertices = voxs(:,1:3);
Surft.Is(ind) = max(Surft.Is)+1;
[Surfgl] = Struct_Area_Comp(Surft);


ind = find(ismember(Surfa(2).Is(:,2), [250 251 252 253 254 255 4 28 5 30 31 SubCL ]'));
Surfa(2).Is(ind,2) = 0;
Surftemp.SurfData.vertices = Surfa(2).SurfData.vertices;
Surftemp.SurfData.faces = Surfa(2).SurfData.faces;
Surftemp.Is = Surfa(2).Is(:,2);
[Surft] = Surf_Corr(Surftemp);
voxs = (V.mat*[Surft.SurfData.vertices(:,1) Surft.SurfData.vertices(:,2) Surft.SurfData.vertices(:,3) ones(size(Surft.SurfData.vertices(:,1),1),1)]')';
Surft.SurfData.vertices = voxs(:,1:3);
Surft.Is(ind) = max(Surft.Is)+1;
[Surfgr] = Struct_Area_Comp(Surft);
%% ===================================================================== %%
return
