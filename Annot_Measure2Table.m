function OutStatFile = Annot_Measure2Table(FreeSDir, Idfile, OutStatFile, annotIds, thtype);
%
% Syntax :
...
if exist(Idfile,'file')
  ...
  FreeSDir = '/media/Data/PROCESSING_RESULTS/URNC/Longitudinal_analysis/Structural/5-freesurfer_processing/New_annot/subjects_long_temp';
  OutStatFile = '/media/Data/PROCESSING_RESULTS/URNC/Longitudinal_analysis/Structural/5-freesurfer_processing/New_annot/subjects_long_temp/AnnotStat.txt';
%  Ids = '0822x-20110430';
...
for i = 1:Nc
    ...
    for z= 1:Nsubj
        ...
        % Local Gyrification Index Maps
        lhlgif = [FreeSDir filesep subjId filesep 'surf' filesep 'lh.pial_lgi'];
        rhlgif = [FreeSDir filesep subjId filesep 'surf' filesep 'rh.pial_lgi'];
        
        % Volumetric Atlas
        atlasfile = [FreeSDir filesep subjId filesep 'mri' filesep aparcId '+aseg.mgz'];
        
        % Out Hull Surface
        outhsurf = [FreeSDir filesep subjId filesep 'surf' filesep subjId '_hullsurf.mat'];
        ...
        %----- Reading Local Gyrification Index Map
        [left_lgi,ctab] = read_cfiles(lhlgif);
        [right_lgi,ctab] = read_cfiles(rhlgif);
        ...
        voxsize = sqrt(sum(VA.mat(1:3,1:3).^2))/10; % cm
        ...
        for j = 1:Nr
            ...
            llgi(j,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        lhlgi(1,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        llobelgi(1,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        llobelgi(2,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        llobelgi(3,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        llobelgi(4,1) = mean(left_lgi(ind)); % Mean Local gyrification index
        ...
        for j = 1:Nr
            ...
            rlgi(j,1) = mean(right_lgi(ind)); % Mean Local gyrification index
            %----- Computing Area
        ...
        rhlgi(1,1) = mean(right_lgi(ind)); % Mean Local gyrification index
        ...
        rlobelgi(1,1) = mean(right_lgi(ind)); % Mean Local gyrification index
        ...
        rlobelgi(2,1) = mean(right_lgi(ind)); % Mean Local gyrification index
        ...
        rlobelgi(3,1) = mean(right_lgi(ind)); % Mean Local gyrification index
        ...
        rlobelgi(4,1) = mean(right_lgi(ind)); % Mean Local gyrification index
...
%         [Surfl,Surfr] = Create_Region_ConvexHull(VA.fname);
        Surfwl.Is = txtl;
        [Surfl] = Create_Region_ConvexHull_From_Surface(Surfwl);
        
        for j =1:size(ctabl.table,1)
            ind = find(Surfl.Is == ctabl.table(j,5));
            Surfl.Is(ind) = ctabl.table(j,6);
        end
        [Surfl] = Struct_Area_Comp(Surfl);
        
        
        Surfwr.Is = txtr;
        [Surfr] = Create_Region_ConvexHull_From_Surface(Surfwr);
        
        for j =1:size(ctabr.table,1)
            ind = find(Surfr.Is == ctabr.table(j,5));
            Surfr.Is(ind) = ctabr.table(j,6);
        end
        [Surfr] = Struct_Area_Comp(Surfr);
        ...
        cortlgi = mean([left_lgi(indl);right_lgi(indr)]); % Mean Local gyrification index
        ...
        LGyrifNames = {'Total_LocGyrifIndex';'LH_LocGyrifIndex';'RH_LocGyrifIndex'};
        Temp = [cellstr([repmat('LH_LocGyrifIndex-',[4 1]) strvcat('Frontal','Parietal','Temporal','Occipital')]) cellstr([repmat('RH_LocGyrifIndex-',[4 1]) strvcat('Frontal','Parietal','Temporal','Occipital')])]';
        Temp= Temp(:);
        Temp1 = [cellstr([repmat('LH_LocGyrifIndex-',[size(ctabl.struct_names,1) 1]) char(ctabl.struct_names)]) cellstr([repmat('RH_LocGyrifIndex-',[size(ctabr.struct_names,1) 1]) char(ctabr.struct_names)])]';
        Temp1= Temp1(:);
        LGyrifNames = [LGyrifNames;Temp;Temp1];
        lobeslgis = [llobelgi rlobelgi]';lobeslgis = lobeslgis(:);
        roislgis = [llgi rlgi]';roislgis = roislgis(:);
        LGyrifIndex = [cortlgi;lhlgi;rhlgi;lobeslgis;roislgis];
   
        for k=1:size(CurvNames,1);
            temp = [deblank(char(VCortNames(k))) '(cm^3)'];
            VCortNames{k} = temp;
            temp = [deblank(char(ACortNames(k))) '(cm^2)'];
            ACortNames{k} = temp;
            temp = [deblank(char(CortThickNames(k))) '(mm)'];
            CortThickNames{k} = temp;
            temp = [deblank(char(CortThickSNames(k))) '(mm)'];
            CortThickSNames{k} = temp;
            temp = [deblank(char(CurvNames(k))) '(mm)'];
            CurvNames{k} = temp;
            temp = [deblank(char(HullNames(k))) '(cm^2)'];
            HullNames{k} = temp;
        end
        
        
        cadvarname = 'Subject_ID;Intracranial_Volume(cm^3);Total_Brain_Volume(cm^3)';
        cadvars = [subjId ';' num2str(ICV) ';' num2str(TBV)];
        for j = 1:length(CortCurv)
            cadvarname = [cadvarname ';' char(NvertNames(j)) ';' char(VCortNames(j)) ';' char(ACortNames(j)) ';' char(CortThickNames(j)) ';' char(CortThickSNames(j)) ';' char(CurvNames(j)) ';' char(HullNames(j))  ';' char(RGyrifNames(j))  ';' char(LGyrifNames(j)) ];
            cadvars = [cadvars ';' num2str(Numvert(j)) ';' num2str(CortVol(j)) ';' num2str(CortArea(j)) ';' num2str(CortCthM(j)) ';' num2str(CortCthS(j)) ';' num2str(CortCurv(j))  ';' num2str(HullSurf(j)) ';' num2str(RGyrifIndex(j)) ';' num2str(LGyrifIndex(j))];
        end
            CadTotal = strvcat(cadvarname,cadvars);
            OStatFile = [FreeSDir filesep subjId filesep 'stats' filesep subjId '_whole_freesurfer_stats_' aparcId '.txt'];
