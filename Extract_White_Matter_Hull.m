function [Surfl,Surfr] = Extract_White_Matter_Hull(Image)
%
% Syntax :
...
% T = '/media/Data/PROCESSING_RESULTS/5-freesurfer_processing/1Test_HCP_899885-20140807-T1wMPR1/mri/wmparc.mgz';
% Image = '/media/Data/PROCESSING_RESULTS/5-freesurfer_processing/1Test_HCP_899885-20140807-T1wMPR1/tmp/wmparc.nii';
% cad = ['mri_convert -i ' T ' -o ' Image];
% system(cad);
...
switch atype
    case 'aparc+aseg'
        ...
        FroIds = sort([1028 1003 1027 1018 1019 1020 1012 1014 1024 1017 1032])+2000;
        FroIdsR = FroIds+1000;
        
        % Selecting Temporal regions
        TempIds = sort([1009 1015 1030 1001 1007 1034 1006 1033 1016])+2000;
        TempIdsR = TempIds+1000;
        
        % Selecting Parietal regions
        ParIds = sort([1029 1008 1031 1022 1025])+2000;
        ParIdsR = ParIds+1000;
        
        % Selecting Occipital regions
        OccIds = sort([1011 1013 1005 1021])+2000;
        OccIdsR = OccIds+1000;
        
        % Selecting Cingulate regions
        CingIds = sort([1002 1010 1023 1026])+2000;
...
ind = find(ismember(I,[ 4 28 5 30 31 SubCL FroIds TempIds ParIds OccIds 3000  3002 3010 3023 3026 3035 ]'));
...
%----------------------------------
ind = find(ismember(I,[43 44 60 62 63 SubCR FroIdsR TempIdsR ParIdsR OccIdsR 4000  4002 4010 4023 4026 4035 ]'));
...
IT = IA*0;
ind = find(ismember(I,[ 4 28 5 30 31 SubCL FroIds TempIds ParIds OccIds 3000  3002 3010 3023 3026 3035 250 251 252 253 254 255]'));
IT(ind) = I(ind);
    ...
    if sum(A(:))
        ...
        for j = 1:length(B)
            ...
            while (isequal(ind,indold) ==0)|~isempty(ind)
                ...
                indr = ismember(A(indid),[3000]);
                
                %indr = ismember(A(indid),[3001 3014 3000 3006 3016 3021 7000 3032 3033 3034 3035]);
                ind = find((k >0)&indr==0);
                boundary(ind,:) = [];
            end
...
IT = IA*0;
T = I(:);
ind = ismember(T,[ 4 28 5 30 31 SubCR FroIdsR TempIdsR ParIdsR OccIdsR 4000  4002 4010 4023 4026 4035 250 251 252 253 254 255]');
IT(ind) = I(ind);
[IT] = Atlas_Corr_custom(It,IT);
...
ind = find(ismember(I,[ 4 28 5 30 31 SubCR FroIdsR TempIdsR ParIdsR OccIdsR 4000  4002 4010 4023 4026 4035 250 251 252 253 254 255 41 5002]'));
Itemp = I*0;
Itemp(ind) = I(ind);
IsL = logical(IT*0);
IsL(ind) = 1;
